...
1. Clone the repository:
    ```powershell
    git clone https://github.com/hailo-ai/hailo-apps.git
    cd hailo-apps\hailo_apps\python\standalone_apps\super_resolution
    ```
...
```shell script
python .\super_resolution.py -n <model_path> -i <input_image_path> -o <output_path>
```
...
- `-t`, `--timeout`: [optional] Maximum runtime in seconds. Stops the application after the specified duration.
